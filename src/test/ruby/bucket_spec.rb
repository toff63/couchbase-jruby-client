describe Couchbase::Bucket, :cluster => true do
  ...
  specify 'append string' do
    upsert = bucket.upsert(Couchbase::Document.new(:id => 'append', :content => 'foo', :transcode => false))
    expect(upsert).not_to be_nil
    expect(upsert.id).to eq('append')

    bucket.append(Couchbase::Document.new(:id => 'append', :content => 'bar', :transcode => false))
  ...
  specify 'prepend string' do
    upsert = bucket.upsert(Couchbase::Document.new(:id => 'prepend', :content => 'foo', :transcode => false))
    expect(upsert).not_to be_nil
    expect(upsert.id).to eq('prepend')

    bucket.prepend(Couchbase::Document.new(:id => 'prepend', :content => 'bar', :transcode => false))
